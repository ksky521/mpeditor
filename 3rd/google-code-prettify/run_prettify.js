...
(function () {
    ...
    let head = doc.head || doc.getElementsByTagName('head')[0] || root;
    ...
    function contentLoaded(callback) {
        ...
        let done = false; let top = true;
        let add = addEventListener ? 'addEventListener' : 'attachEvent';
        let rem = addEventListener ? 'removeEventListener' : 'detachEvent';
        let pre = addEventListener ? '' : 'on';
    ...
    let LOADER_BASE_URL
    = 'https://raw.githubusercontent.com/google/code-prettify/master/loader';
     ...
     */
      ...
      + 'static_cast,template,typeid,typename,using,virtual,where'];
        ...
        + 'throws,transient'];
            let CSHARP_KEYWORDS = [COMMON_KEYWORDS,
        ...
        + 'unchecked,unsafe,ushort,var,virtual,where'];
            let COFFEE_KEYWORDS = 'all,and,by,catch,class,else,extends,false,finally,'
      ...
      + 'False,True,None'];
            let RUBY_KEYWORDS = [FLOW_CONTROL_KEYWORDS, 'alias,and,begin,case,class,'
      ...
      + 'function,in,local,set,then,until'];
            ...
            let C_TYPES = /^(DIR|FILE|vector|(de|priority_)?queue|list|stack|(const_)?iterator|(multi)?(set|map)|bitset|u?(int|float)\d*)\b/;
       ...
       * @const
       */
            let REGEXP_PRECEDER_PATTERN = '(?:^^\\.?|[+-]|[!=]=?=?|\\#|%=?|&&?=?|\\(|\\*=?|[+\\-]=|->|\\/=?|::?|<<?=?|>>?>?=?|,|;|\\?|@|\\[|~|{|\\^\\^?=?|\\|\\|?=?|break|case|continue|delete|do|else|finally|instanceof|return|throw|try|typeof)\\s*';
       ...
       */
         ...
         */
                var decorate = function (job) {
                    let sourceCode = job.sourceCode; let basePos = job.basePos;
                    let sourceNode = job.sourceNode;
       ...
       */
            function sourceDecorator(options) {
                let shortcutStylePatterns = []; let fallthroughStylePatterns = [];
                ...
                let regexLiterals = options.regexLiterals;
                if (regexLiterals) {
                    /**
           * @const
           */
                    let regexExcls = regexLiterals > 1
                        ? '' // Multiline regex literals
                        : '\n\r';
                    /**
           * @const
           */
                    let regexAny = regexExcls ? '.' : '[\\S\\s]';
                    /**
           * @const
           */
                    let REGEX_LITERAL = (
          ...
          + '/');
                ...
                let keywords = ('' + options.keywords).replace(/^ | $/g, '');
                if (keywords.length) {
                    fallthroughStylePatterns.push(
                        [PR_KEYWORD,
            ...
            function numberLines(node, startLineNum, isPreformatted) {
                ...
                function walk(node) {
                    ...
                    if (type == 1 && !nocode.test(node.className)) { // Element
                        ...
                        let text = node.nodeValue;
                        let match = text.match(lineBreak);
                        if (match) {
                            let firstLine = text.substring(0, match.index);
                ...
                function breakAfter(lineEndNode) {
                    ...
                    function breakLeftOf(limit, copy) {
                        ...
                        let rightSide = copy ? limit.cloneNode(false) : limit;
                        let parent = limit.parentNode;
                        if (parent) {
                            // We clone the parent chain.
                            // This helps us resurrect important styling elements that cross lines.
                            // E.g. in <i>Foo<br>Bar</i>
                            // should be rewritten to <li><i>Foo</i></li><li><i>Bar</i></li>.
                            let parentClone = breakLeftOf(parent, 1);
                ...
                let offset = Math.max(0, ((startLineNum - 1 /* zero index */)) | 0) || 0;
       ...
       */
            function recombineTagsAndDecorations(job) {
                ...
                let source = job.sourceCode;
                let sourceLength = source.length;
                // Index into source after the last code-unit recombined.
                let sourceIndex = 0;

                let spans = job.spans;
                let nSpans = spans.length;
                ...
                for (i = decPos = 0; i < nDecorations;) {
                    let startPos = decorations[i];
                    // Conflate all adjacent decorations that use the same style.
                    let startDec = decorations[i + 1];
                    var end = i + 2;
                ...
                try {
                    ...
                    while (spanIndex < nSpans) {
                        let spanStart = spans[spanIndex];
                        let spanEnd = /** @type{number} */ (spans[spanIndex + 2])
              || sourceLength;

                        let decEnd = decorations[decorationIndex + 2] || sourceLength;
              ...
              && (styledText = source.substring(sourceIndex, end))) {
                            ...
                            let document = textNode.ownerDocument;
       ...
       */
            ...
            function applyDecorator(job) {
                ...
                try {
                    ...
                    let sourceAndSpans = extractSourceSpans(job.sourceNode, job.pre);
                    /** Plain text. @type {string} */
                    let source = sourceAndSpans.sourceCode;
       ...
       */
            function $prettyPrintOne(sourceCodeHtml, opt_langExtension, opt_numberLines) {
                /** @type{number|boolean} */
                let nl = opt_numberLines || false;
                /** @type{string|null} */
                let langExtension = opt_langExtension || null;
                /** @type{!Element} */
       ...
       */
            function $prettyPrint(opt_whenDone, opt_root) {
                let root = opt_root || document.body;
                ...
                let langExtensionRe = /\blang(?:uage)?-([\w.]+)(?!\S)/;
                let prettyPrintRe = /\bprettyprint\b/;
                let prettyPrintedRe = /\bprettyprinted\b/;
                let preformattedTagNameRe = /pre|xmp/i;
                let codeRe = /^code$/i;
                let preCodeXmpRe = /^(?:pre|code|xmp)$/i;
                let EMPTY = {};

                function doWork() {
                    let endTime = (win.PR_SHOULD_USE_CONTINUATION
                        ? clock.now() + 250
                        : /* ms */ Infinity);
                    for (; k < elements.length && clock.now() < endTime; k++) {
                        ...
                        var attrs = EMPTY;
                        {
                            for (let preceder = cs; (preceder = preceder.previousSibling);) {
                                let nt = preceder.nodeType;
                                // <?foo?> is parsed by HTML 5 to a comment node (8)
                                // like <!--?foo?-->, but in XML is a processing instruction
                                let value = (nt === 7 || nt === 8) && preceder.nodeValue;
                                if (value
                  ...
                  && p.className && prettyPrintRe.test(p.className)) {
                    ...
                    && codeRe.test(wrapper.tagName)) {
                                ...
                                if (preformattedTagNameRe.test(cs.tagName)) {
                                    ...
                                    let currentStyle = cs.currentStyle;
